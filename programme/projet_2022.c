...
//reçoit un tableau 2D, une position (celle de la reine choisie), et le joueur qui joue
...
//reçoit unt tableau 2D et le numéro du joueur dont c'est le tour
...
//reçoit une position de départ et d'arrivée, et un tableau 2D
//détermine si le déplacement est autorisé
...
int main(void) {
    ...
    int winner = 0; //joueur gagnant (0 s'il n'y en a pas encore)
